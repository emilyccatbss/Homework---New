...
void turtleDraw() {
  ...
  // Set the line length for this round of drawing
  currentLineLength = lineLength/2;
  ...
  for (currentCharacter= 0; currentCharacter < newWord.length(); currentCharacter++) {
    ...
    if ( newWord.charAt(currentCharacter) == '+') {
      ...
      // saving current position and rotation info onto stack
      pushMatrix();
    }
    else if ( newWord.charAt(currentCharacter) == ']') {
      // remove most recently saved location and rotation info on stack
      popMatrix();
    }
    else {
      // Draw a segment
