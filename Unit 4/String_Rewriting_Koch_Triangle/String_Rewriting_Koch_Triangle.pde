...
void rewriteWord() {
  ...
  for (currentCharacter= 0; currentCharacter < existingWord.length(); currentCharacter++) {
    ...
    if ( existingWord.charAt(currentCharacter) == 'A') {
      // replace
      newWord += rules.get("A");
      fill(255, 0, 0); 
      fill(0, 0, 0);
    } 
    else if ( existingWord.charAt(currentCharacter) == 'B') {
      // replace
      newWord += rules.get("B");
      fill(255, 0, 0); 
      fill(0, 0, 0);
...
void turtleDraw() {
  ...
  currentLength = initialLength/pow(4, rewriteCount);
  for (currentCharacter= 0; currentCharacter < newWord.length(); currentCharacter++) {

    if (newWord.charAt(currentCharacter) == 'A') {

      line(0, 0, currentLength, 0);
      translate(currentLength, 0);
    } 
     else if (newWord.charAt(currentCharacter) == 'B') {
